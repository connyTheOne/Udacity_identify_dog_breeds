...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       #  Runs classifier function to classify the images classifier function inputs: path + filename  and  model, returns model_label as classifier label
       # Processes the results so they can be compared with pet image labels set labels to lowercase (lower) and stripping off whitespace(strip)
       model_label = classifier(images_dir +'/'+ key, model).lower().strip()
       results_dic[key].extend([model_label])
       # defines truth as pet image label 
       truth = results_dic[key][0]

       # If the pet image label is found within the classifier label list of terms as an exact match to on of the terms in the list - then they are added to 
